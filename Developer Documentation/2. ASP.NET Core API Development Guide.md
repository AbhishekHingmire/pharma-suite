...
```
PharmaSuite/
├── PharmaSuite.API/                    # Web API Project
│   ├── Controllers/
│   ├── Middleware/
│   ├── Program.cs
│   └── appsettings.json
│
├── PharmaSuite.Core/                   # Domain Layer
│   ├── Entities/                       # Database models
│   ├── Interfaces/                     # Repository interfaces
│   ├── DTOs/                          # Data Transfer Objects
│   └── Constants/
│
├── PharmaSuite.Infrastructure/         # Data Access Layer
│   ├── Data/                          # DbContext
│   ├── Repositories/                  # Repository implementations
│   └── Migrations/
│
└── PharmaSuite.Services/              # Business Logic Layer
...
### 6.9 EmployeesController (HR Module - Two-Tier System)

**Note**: The HR Module has a two-tier architecture:
- **Basic HR** (Always active): Employee management, activity tracking, performance metrics
- **Full HR** (Subscription-based): Attendance, leave, shift management

**Endpoints:**
- `GET /api/employees` - Get all employees (filters: department, status, role)
- `GET /api/employees/{id}` - Get employee by ID
- `POST /api/employees` - Create employee (admin only)
- `PUT /api/employees/{id}` - Update employee (admin only)
- `DELETE /api/employees/{id}` - Delete employee (admin only)
- `GET /api/employees/{id}/timeline` - Get employee activity timeline
- `GET /api/employees/{id}/activities` - Get activities with filters (today/week/month)
...
public class EmployeeDto
{
    ...
    [Required]
    public EmployeeRole EmployeeRole { get; set; } // sales-rep, warehouse-manager, etc.
    
    [Required, MaxLength(20)]
    public string EmployeeCode { get; set; }
    
    [Required]
    public Department Department { get; set; } // sales, warehouse, operations, accounts, quality
    
    [Required]
    public EmployeeStatus Status { get; set; } // active, inactive, on-leave
    ...
    public List<DayOfWeek> WeeklyOff { get; set; } // e.g., [Sunday]
}

public class WorkingHours
{
    public TimeSpan Start { get; set; } // e.g., 09:00
    public TimeSpan End { get; set; }   // e.g., 18:00
}
...
// Demo Data Generation: 180 days (6 months) with realistic variations
...
### 6.10 AttendanceController (Full HR - Subscription Required)

**Endpoints:**
- `GET /api/attendance` - Get attendance records (filters: employee, date range)
...
- `GET /api/attendance/calendar/{employeeId}` - Get calendar view (30 days)
...
public class AttendanceDto
{
    ...
    public string MarkedBy { get; set; } // admin, system, or userId
    public string Notes { get; set; }
}
...
## 12. Frontend Integration Notes (React + TypeScript)

### 12.1 Recent Frontend Optimizations (November 2025)

The React frontend has been optimized for better mobile UX and performance:

**Employee Management Optimizations:**
1. **Card Layout**: Reduced padding (p-3 → p-2), smaller avatars (w-10 → w-8) for 20-25% height reduction on mobile
2. **Visible Borders**: Added border-t separators between card sections (header, info, actions) for better readability
3. **Filter Layout**: 2-column grid for department + status filters on all devices
4. **Breadcrumb Fix**: `/masters/employees` now shows "Home > Employees" (not "Masters > Employees")
5. **Navigation Fix**: Masters nav item no longer highlights when on Employees page (exact matching with `end` prop)

**Timeline Enhancements:**
1. **Long Data**: 180 days (6 months) of activity data with realistic variations:
   - High activity employees: 3-5 activities/day (~900+ total)
   - Medium activity employees: 2-3 activities/day (~540+ total)
   - Low activity employees: 1-2 activities/day (~270+ total)
2. **Scrollable**: `max-h-[600px] overflow-y-auto` for long activity lists
3. **Activity Types**: Visit, Call, Meeting, Task, Delivery with type-specific details

**Attendance Calendar:**
1. **Interactive**: Clickable dates show daily attendance view
2. **Optimized Layout**: Scaled to 90%, 5-column grid (calendar 2 cols, list 3 cols)
3. **Stats**: Total, Present, Absent, Leave (Late badge in cards when check-in > 09:15)

**Sidebar Navigation:**
1. **Scrolling Fix**: Settings and Logout now inside scrollable area (not stuck at bottom)
2. **Two-Tier HR**: Basic HR always visible, Full HR shows "Pro" badge when subscription-gated

### 12.2 API Response Expectations
...
```json
{
  ...
  "activities": [
    {
      ...
      "type": "visit",
      "title": "Customer Visit - Apollo Pharmacy",
      "description": "Product demonstration and order discussion. Order value: ₹45000",
      "status": "completed",
      "visitDetails": {
  ...
  "stats": {
    "sales": { "count": 15, "value": 250000 },
    "purchases": { "count": 8, "value": 180000 },
    "attendance": { "present": 22, "total": 30, "percentage": 73 },
    "activities": 85
  }
}
```

**Attendance Calendar API Response:**
```json
{
  "month": "2025-11",
  "records": [
...
### 12.3 Auto-Activity Logging

Activities are automatically logged for inventory operations:

```typescript
// When sale is created:
POST /api/employees/{employeeId}/activities
{
  "type": "sale",
  "title": "Sale Created - INV001",
  "description": "Invoice for Apollo Pharmacy - ₹25000",
  "saleId": 123,
  "customerId": 45,
  "status": "completed"
}
...
{
  "type": "purchase",
  "title": "Purchase Created - PUR001",
  "description": "Order from ABC Pharma - ₹50000",
  "purchaseId": 78,
  "companyId": 12,
  "status": "completed"
}
...
### 12.4 HR Tier Checking

Frontend uses these helper functions:
```typescript
// Always returns true - Basic HR is free
isBasicHREnabled(): boolean

// Checks subscription status
isFullHREnabled(): boolean
```

API should provide subscription info in auth response:
```json
{
  "userId": 1,
  "token": "...",
...
**Estimated Development Time**: 6-8 weeks (including HR Module)  
...
- Added HR Module (Two-Tier System) - Employees, Attendance, Activities
- Added Frontend Integration Notes section
- Added Auto-Activity Logging implementation
- Added Timeline API with 6 months of realistic data patterns
- Added Attendance Calendar API specifications
- Updated Employee models with HR-specific fields
- Added subscription-based feature gating
- Updated development time estimate (4-6 weeks → 6-8 weeks)
